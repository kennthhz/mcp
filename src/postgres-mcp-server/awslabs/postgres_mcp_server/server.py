...
async def run_query(
    ...
    issue = check_sql_injection_risk(sql)
    if issue:
        logger.info(
            f'query is rejected because it contains risky SQL pattern, SQL query: {sql}, reasons: {issue}'
        )
        await ctx.error(
            str({'message': 'Query parameter contains suspicious pattern', 'details': issue})
        )
...
@mcp.tool(
    ...
    description='Fetch table columns and comments from a Postgres database',
)
...
def main():
    ...
    try:
        ...
        logger.exception(
            'Failed to validate RDS API db connection to Postgres. Exit the MCP server'
        )
        sys.exit(1)
